import playaImage from '../../../../../assets/playa.png';
import { useApp } from '../../../context/AppContext';

export function BigBanner() {
    const { navigate, darkMode } = useApp();
    return (
        <section className={`pb-20 px-4 sm:px-8 ${darkMode ? 'bg-black' : 'bg-white'}`}>
            <div className="max-w-7xl mx-auto">
                <div className={`relative h-96 sm:h-[500px] overflow-hidden ${darkMode ? 'bg-white' : 'bg-black'}`}>
                    ...
                    <div className={`absolute inset-0 flex flex-col items-center justify-center text-center px-4 ${
                        ...
                        <h2 className={`text-4xl sm:text-6xl mb-6 ${darkMode ? 'text-black' : 'text-white'}`}>Playa y Mar</h2>
                        <button
