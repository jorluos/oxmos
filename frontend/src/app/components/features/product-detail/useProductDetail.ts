...
export function useProductDetail(product?: Product) {
  ...
  const currentColorName = colors.find((c: { hex: string; name: string }) => c.hex === currentColorHex)?.name || colors[0]?.name || '';
  ...
  const selectedVariant = useMemo(() => {
    ...
    return (
      product.variants?.find(
        (v: ProductVariant) => v.size === selectedSize && (!currentColorHex || v.color_hex === currentColorHex)
      ) ?? null
    );
  ...
  const handleAddToCart = async () => {
    ...
    const variant = selectedVariant ?? product.variants?.find((v: ProductVariant) => v.size === selectedSize);
    if (!variant) return;
  ...
  const handleBuyNow = () => {
    ...
    const variant = selectedVariant ?? product.variants?.find((v: ProductVariant) => v.size === selectedSize && v.is_active);
    if (!variant) return;
